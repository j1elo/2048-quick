...
void BoardUi::setGameManager(GameManager* game)
{
    ...
    this->disconnect();
    if (game) {
        connect(game, SIGNAL(valueAdded(int,int)), this, SLOT(addTile(int,int)));
        connect(game, SIGNAL(valueMoved(int,int)), this, SLOT(moveTile(int,int)));
        connect(game, SIGNAL(valueChanged(int,int)), this, SLOT(setTile(int,int)));
...
void BoardUi::setTile(int index, int value)
{
    Q_ASSERT(m_root);
    QMetaObject::invokeMethod(m_root, "setTile",
        Q_ARG(QVariant, index), Q_ARG(QVariant, value));
}
